...
/** Ford BARRA - a dedicated decoder for Ford Barra 6cyl engine.
Standard 36-1 trigger wheel running at crank speed and 6-2 trigger wheel running at cam speed.
* @defgroup dec_ford_st170 Ford ST170 (01-04 Focus)
* @{
*/
void triggerSetup_FordBARRA(void)
{
  ...
  triggerSecFilterTime = 1000000 * 60 / MAX_RPM / 6 / 2; //Cam pattern is 6-2, so 2 nearest teeth are 120 deg crank angle apart. Cam can be advanced by 60 deg, so going from fully retarded to fully advanced closes the gap to 30 deg. Zetec cam pulleys aren't keyed from factory, so I subtracted additional 10 deg to avoid filter to be too aggressive. And there you have it 720/20=36.
...
void triggerSetEndTeeth_FordBARRA(void)
{
  ...
  int16_t tempIgnition5EndTooth;
  tempIgnition5EndTooth = ( (ignition5EndAngle - configPage4.triggerAngle) / (int16_t)(triggerToothAngle) ) - 1;
  if(tempIgnition5EndTooth > (36 + toothAdder)) { tempIgnition5EndTooth -= (36 + toothAdder); }
  if(tempIgnition5EndTooth <= 0) { tempIgnition5EndTooth += (36 + toothAdder); }
  if((uint16_t)tempIgnition5EndTooth > (triggerActualTeeth + toothAdder)) { tempIgnition5EndTooth = (triggerActualTeeth + toothAdder); }
  ignition5EndTooth = tempIgnition5EndTooth;

  int16_t tempIgnition6EndTooth;
  tempIgnition6EndTooth = ( (ignition6EndAngle - configPage4.triggerAngle) / (int16_t)(triggerToothAngle) ) - 1;
  if(tempIgnition6EndTooth > (36 + toothAdder)) { tempIgnition6EndTooth -= (36 + toothAdder); }
  if(tempIgnition6EndTooth <= 0) { tempIgnition6EndTooth += (36 + toothAdder); }
  if((uint16_t)tempIgnition6EndTooth > (triggerActualTeeth + toothAdder)) { tempIgnition6EndTooth = (triggerActualTeeth + toothAdder); }
  ignition6EndTooth = tempIgnition6EndTooth;
  // Removed ign channels >6 as a Barra engine is a 6 cylinder
